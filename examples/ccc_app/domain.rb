...
module CourseApp
  ...
  # Partition by :course_name so CCC.load reads all CourseCreated with that name.
  class CourseDecider < CCC::Decider
  ...
  # Partition by :course_id so CCC.load reads CourseCreated + StudentEnrolled for that course.
  class EnrolmentDecider < CCC::Decider
  ...
  # Registered with CCC for background processing by workers.
  class CourseCatalogProjector < CCC::Projector::EventSourced
    partition_by :course_id
    ...
    state do |partition_values|
      ...
      # course_id = partition_values&.first
      # existing = self.class.read_course(course_id) if course_id
      # if existing
      #   { course_id: existing[:course_id], course_name: existing[:course_name], students: Array(existing[:students]).dup }
      # else
      #   { course_id: nil, course_name: nil, students: [] }
      # end
    end
