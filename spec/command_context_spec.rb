...
module CccContextTest
  Add = Sourced::Command.define('ccc_ctest.add') do
    attribute :value, Integer
  end

  Remove = Sourced::Command.define('ccc_ctest.remove') do
    attribute :value, Integer
  end

  Added = Sourced::Event.define('ccc_ctest.added')
end
...
RSpec.describe Sourced::CommandContext do
  describe '#build' do
    ...
    it 'allows scoping to a custom command subclass' do
      custom_scope = Class.new(Sourced::Command)
  ...
  describe 'callback hooks' do
    ...
    it 'registers on block for multiple command types' do
      klass = Class.new(described_class)
      klass.on(CccContextTest::Add, CccContextTest::Remove) { |_app, cmd| cmd.with_metadata(tagged: true) }

      ctx = klass.new
      add_cmd = ctx.build(CccContextTest::Add, payload: { value: 1 })
      remove_cmd = ctx.build(CccContextTest::Remove, payload: { value: 2 })
      expect(add_cmd.metadata[:tagged]).to eq(true)
