...
module DispatcherTestMessages
  ...
  DeviceBound = Sourced::Message.define('dispatch_test.device.bound') do
    attribute :device_id, String
    attribute :asset_id, String
  end

  BindDevice = Sourced::Message.define('dispatch_test.bind_device') do
    attribute :device_id, String
    attribute :asset_id, String
  end

  DelayedNotify = Sourced::Message.define('dispatch_test.delayed_notify') do
    attribute :device_id, String
  end
end

class DispatchTestDecider < Sourced::Decider
  partition_by :device_id
  consumer_group 'dispatch-test-decider'

  state { |_| { exists: false, bound: false } }

  evolve DispatcherTestMessages::DeviceRegistered do |state, _evt|
    state[:exists] = true
  end

  evolve DispatcherTestMessages::DeviceBound do |state, _evt|
    state[:bound] = true
  end

  command DispatcherTestMessages::BindDevice do |state, cmd|
    raise 'Not found' unless state[:exists]
    raise 'Already bound' if state[:bound]
    event DispatcherTestMessages::DeviceBound, device_id: cmd.payload.device_id, asset_id: cmd.payload.asset_id
  end

  reaction DispatcherTestMessages::DeviceBound do |_state, evt|
    dispatch(DispatcherTestMessages::DelayedNotify, device_id: evt.payload.device_id)
      .at(Time.now + 2)
  end
end

class DispatchTestProjector < Sourced::Projector::StateStored
  partition_by :device_id
  consumer_group 'dispatch-test-projector'

  state { |_| { devices: [] } }

  evolve DispatcherTestMessages::DeviceRegistered do |state, evt|
...
RSpec.describe Sourced::Dispatcher do
  ...
  let(:store) { Sourced::Store.new(db, notifier: notifier) }
  ...
  describe 'Store notifications' do
    it 'append triggers notify_new_messages' do
      ...
      store.append(
        DispatcherTestMessages::DeviceRegistered.new(payload: { device_id: 'd1', name: 'Sensor' })
      )
    end
  ...
  describe 'batch_size' do
    it 'claim_next with batch_size limits returned messages' do
      ...
      5.times do |i|
        store.append(
          DispatcherTestMessages::DeviceRegistered.new(payload: { device_id: 'd1', name: "Sensor #{i}" })
        )
      end
  ...
  describe Sourced::Dispatcher::NotificationQueuer do
    let(:queuer) do
      described_class.new(
  ...
  describe Sourced::Worker do
    ...
    describe '#drain' do
      it 'processes until no more work for reactor' do
        store.append(
          DispatcherTestMessages::DeviceRegistered.new(payload: { device_id: 'd1', name: 'Sensor A' })
        )
        store.append(
          DispatcherTestMessages::DeviceRegistered.new(payload: { device_id: 'd2', name: 'Sensor B' })
        )
      ...
      it 're-enqueues reactor when max_drain_rounds reached' do
        ...
        store.append(
          DispatcherTestMessages::DeviceRegistered.new(payload: { device_id: 'd1', name: 'A' })
        )
        store.append(
          DispatcherTestMessages::DeviceRegistered.new(payload: { device_id: 'd2', name: 'B' })
        )
  ...
  describe 'scheduled message promotion' do
    it 'promotes delayed reactions into the main log when due' do
      ...
      conds = DispatcherTestMessages::DelayedNotify.to_conditions(device_id: 'd1')
      result = store.read(conds)
      expect(result.messages.map(&:class)).to include(DispatcherTestMessages::DelayedNotify)
    end
  end
end
