...
module CCCTopologyTest
  # --- Messages ---
  CreateWidget = Sourced::CCC::Command.define('ccc_topo.create_widget') do
    attribute :widget_id, String
    attribute :name, String
  end

  WidgetCreated = Sourced::CCC::Event.define('ccc_topo.widget_created') do
    attribute :widget_id, String
    attribute :name, String
  end

  NotifyWidget = Sourced::CCC::Command.define('ccc_topo.notify_widget') do
    attribute :widget_id, String
  end

  WidgetNotified = Sourced::CCC::Event.define('ccc_topo.widget_notified') do
    attribute :widget_id, String
  end

  ArchiveWidget = Sourced::CCC::Command.define('ccc_topo.archive_widget') do
    attribute :widget_id, String
  end

  WidgetArchived = Sourced::CCC::Event.define('ccc_topo.widget_archived') do
    attribute :widget_id, String
  end

  DelayedCmd = Sourced::CCC::Command.define('ccc_topo.delayed_cmd') do
    attribute :widget_id, String
  end

  ScheduleEvent = Sourced::CCC::Event.define('ccc_topo.schedule_event') do
    attribute :widget_id, String
  end

  # --- Decider ---
  class WidgetDecider < Sourced::CCC::Decider
  ...
  # --- Notifier Decider ---
  class NotifierDecider < Sourced::CCC::Decider
    partition_by :widget_id
  ...
  class WidgetListProjector < Sourced::CCC::Projector::StateStored
    partition_by :widget_id
    consumer_group 'CCCTopologyTest::WidgetListProjector'
  ...
  class ReactingProjector < Sourced::CCC::Projector::EventSourced
    partition_by :widget_id
    consumer_group 'CCCTopologyTest::ReactingProjector'
  ...
  class MixedReactingProjector < Sourced::CCC::Projector::EventSourced
    partition_by :widget_id
    consumer_group 'CCCTopologyTest::MixedReactingProjector'
  ...
  class SchedulingDecider < Sourced::CCC::Decider
    partition_by :widget_id
...
RSpec.describe Sourced::CCC::Topology do
  ...
  context 'with WidgetDecider and NotifierDecider' do
    let(:reactors) { [CCCTopologyTest::WidgetDecider, CCCTopologyTest::NotifierDecider] }
    ...
    it 'sets correct group_id on command nodes' do
      ...
      expect(node.group_id).to eq('CCCTopologyTest::WidgetDecider')
    end
    ...
    it 'sets command name from message class' do
      node = find_node('ccc_topo.create_widget')
      expect(node.name).to eq('CCCTopologyTest::CreateWidget')
    end
    ...
    it 'builds automation nodes for reactions' do
      ...
      expect(aut_nodes.map(&:id)).to include(
        'ccc_topo.widget_created-CCCTopologyTest::WidgetDecider-aut'
      )
    end

    it 'sets correct consumes on automation nodes' do
      node = find_node('ccc_topo.widget_created-CCCTopologyTest::WidgetDecider-aut')
    ...
    it 'sets automation name from event class' do
      node = find_node('ccc_topo.widget_created-CCCTopologyTest::WidgetDecider-aut')
      expect(node.name).to eq('reaction(CCCTopologyTest::WidgetCreated)')
    end
  end

  context 'with WidgetListProjector (no reactions)' do
    let(:reactors) { [CCCTopologyTest::WidgetListProjector] }
    ...
    it 'builds a readmodel node' do
      node = find_node('ccc_topology_test.widget_list_projector-rm')
    ...
    it 'sets correct name and group_id on readmodel node' do
      node = find_node('ccc_topology_test.widget_list_projector-rm')
      expect(node.name).to eq('CCCTopologyTest::WidgetListProjector')
      expect(node.group_id).to eq('CCCTopologyTest::WidgetListProjector')
    end
  ...
  context 'with ReactingProjector (catch-all reaction)' do
    let(:reactors) { [CCCTopologyTest::ReactingProjector] }
    let(:rm_id) { 'ccc_topology_test.reacting_projector-rm' }
    let(:aut_id) { 'ccc_topology_test.reacting_projector-aut' }
    ...
    it 'builds a single catch-all automation node' do
      ...
      expect(node.name).to eq('reaction(CCCTopologyTest::ReactingProjector)')
    end
  ...
  context 'with MixedReactingProjector (specific + catch-all reactions)' do
    let(:reactors) { [CCCTopologyTest::MixedReactingProjector] }
    let(:rm_id) { 'ccc_topology_test.mixed_reacting_projector-rm' }
    let(:specific_aut_id) { 'ccc_topo.widget_created-CCCTopologyTest::MixedReactingProjector-aut' }
    let(:catchall_aut_id) { 'ccc_topology_test.mixed_reacting_projector-aut' }
    ...
    it 'catch-all automation is named after the reactor' do
      ...
      expect(node.name).to eq('reaction(CCCTopologyTest::MixedReactingProjector)')
    end
  ...
  context 'with SchedulingDecider (chained dispatch)' do
    let(:reactors) { [CCCTopologyTest::SchedulingDecider] }

    it 'detects dispatch through .at() chain' do
      node = find_node('ccc_topo.schedule_event-CCCTopologyTest::SchedulingDecider-aut')
      expect(node).not_to be_nil
  ...
  context 'event deduplication across reactors' do
    let(:reactors) { [CCCTopologyTest::WidgetDecider, CCCTopologyTest::WidgetListProjector] }
  ...
  context 'command deduplication across reactors' do
    let(:reactors) { [CCCTopologyTest::WidgetDecider, CCCTopologyTest::SchedulingDecider] }
  ...
  context 'with all test reactors' do
    let(:reactors) do
      [
        CCCTopologyTest::WidgetDecider,
        CCCTopologyTest::NotifierDecider,
        CCCTopologyTest::WidgetListProjector,
        CCCTopologyTest::ReactingProjector,
        CCCTopologyTest::SchedulingDecider
      ]
    end
