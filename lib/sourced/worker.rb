...
module Sourced
  # Processes reactors from a {WorkQueue} in a signal-driven drain loop.
  # Calls {Sourced::Router#handle_next_for} for each signaled reactor.
  #
  # @example Signal-driven mode (production)
  #   queue = Sourced::WorkQueue.new(max_per_reactor: 4)
  #   worker = Sourced::Worker.new(work_queue: queue, router: router, name: 'worker-0')
  #   worker.run  # blocks, processing reactors popped from queue
  #
  # @example Single-tick for testing
  #   worker = Sourced::Worker.new(work_queue: queue, router: router, name: 'test')
  ...
  class Worker
    ...
    # @param router [Sourced::Router] router for dispatching messages
