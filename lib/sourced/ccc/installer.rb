...
module Sourced
  module CCC
    class Installer
      ...
      def initialize(db, logger:, prefix: 'sourced', migration_template: '001_create_ccc_tables.rb.erb')
      ...
      def copy_migration_to(dir = nil, &block)
        path = block ? block.call : File.join(dir, '001_create_ccc_tables.rb')
        File.write(path, rendered_migration)
        logger.info("Copied CCC migration to #{path}")
        path
      end
